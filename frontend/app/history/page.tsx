      ...
      <AnimatePresence mode="wait">
        {selectedId === null ? (
          <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-[#f9fafb] mb-1">History</h1>
              <p className="text-[#6b7280] text-sm">Every analysis you've run — pasted code, uploaded files, and PR reviews.</p>
            </div>
            ...
            {entries === null ? (
              ...
              <div className="bg-[#111827] border border-[#374151] rounded-xl p-8 text-center">
                <Clock className="w-8 h-8 text-[#4b5563] mx-auto mb-3" />
                <p className="text-[#f9fafb] font-medium mb-1">No history yet</p>
                <p className="text-sm text-[#6b7280]">Run an analysis or review a PR — it'll show up here.</p>
              </div>
            ) : (
                ...
                {entries.map((e) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 bg-[#111827] border border-[#374151] hover:border-[#4b5563] rounded-xl text-left transition-colors"
                    >
                      <Icon className={`w-4 h-4 flex-shrink-0 ${cfg.color}`} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-[#f9fafb] truncate">{e.label}</p>
                        <p className="text-xs text-[#6b7280]">{cfg.label} · {timeAgo(e.created_at)}</p>
                      </div>
          ...
          <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-[#6b7280] hover:text-[#f9fafb] transition-colors mb-4"
            >
            ...
            {detailLoading || !detail ? (
              ...
              <>
                <div className="mb-6">
                  <h1 className="text-2xl font-bold text-[#f9fafb] mb-1">{detail.label}</h1>
